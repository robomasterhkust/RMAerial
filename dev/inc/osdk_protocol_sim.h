...
typedef struct
{
  ...
  uint8_t                  data[OSDK_MAX_PACKET_LEN - OSDK_NON_PAYLOAD_LEN];
...
typedef struct
{
  ...
  uint16_t  seq;
  uint16_t  crc16;
  uint8_t   data[OSDK_MAX_PACKET_LEN - OSDK_HEADER_LEN];
  //uint32_t  crc32;
...
typedef struct
{
  ...
  float q2; /*!< y */
  float q3; /*!< z */
} __attribute__((packed)) osdk_quaternion;   // pack(1)
...
typedef struct
{
  ...
  float y;
  float z;
} __attribute__((packed)) osdk_vector3f; // pack(1)
...
typedef struct
{
  ...
  int32_t y;
  int32_t z;
} __attribute__((packed)) osdk_vector3d; // pack(1)
...
typedef struct
{
  ...
  uint32_t time_ns;
  uint8_t  sync;
} __attribute__((packed)) osdk_timeStamp; // pack(1)
...
typedef struct
{
  ...
  uint16_t tag;
  uint8_t  flag;
} __attribute__((packed)) osdk_syncStamp; // pack(1)
...
typedef struct
{
  ...
  uint8_t reserve : 7;
} __attribute__((packed)) osdk_velocityInfo; // pack(1)
...
} __attribute__((packed)) osdk_velocity; // pack(1)
...
typedef struct
{
  ...
  uint8_t   health;    /*!< scale from 0 - 5 signifying gps signal strength <br>
                        * greater than 3 for strong signal */
} __attribute__((packed)) osdk_globalPosition;      // pack(1)
...
} __attribute__((packed)) osdk_GPSFused;                         // pack(1)
...
} __attribute__((packed)) osdk_relativePosition; // pack(1)
...
} __attribute__((packed)) osdk_positionTimeStamp; // pack(1)
...
} __attribute__((packed)) osdk_positionData;        // pack(1)
...
} __attribute__((packed)) osdk_GPSDetail;            // pack(1)
...
} __attribute__((packed)) osdk_GPSInfo; // pack(1)
...
typedef struct
{
  ...
  int16_t y;
  int16_t z;
} __attribute__((packed)) osdk_mag; // pack(1)
...
} __attribute__((packed)) osdk_RC;               // pack(1)
...
} __attribute__((packed)) osdk_gimbalStatus; // pack(1)
...
typedef struct
{
  ...
  uint8_t   reserved : 5;
} __attribute__((packed)) osdk_gimbal; // pack(1)
...
} __attribute__((packed)) osdk_status;         // pack(1)
...
typedef struct
{
  ...
  int32_t  current;
  uint8_t  percentage;
} __attribute__((packed)) osdk_battery; // pack(1)
...
typedef struct
{
  ...
  uint8_t vrcStatus : 1;
  uint8_t reserved : 3;
} __attribute__((packed)) osdk_Info; // pack(1)
