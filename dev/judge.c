...
const unsigned char CRC8_INIT_JUDGE = 0xff;
...
unsigned int Verify_CRC8_Check_Sum(unsigned char *pchMessage, unsigned int dwLength)
{
    ...
    if ((pchMessage == 0) || (dwLength <= 2))
        return 0;
    ucExpected = Get_CRC8_Check_Sum (pchMessage, dwLength - 1, CRC8_INIT_JUDGE);
...
void Append_CRC8_Check_Sum(unsigned char *pchMessage, unsigned int dwLength)
{
    ...
    if ((pchMessage == 0) || (dwLength <= 2))
        return;
    ucCRC = Get_CRC8_Check_Sum ( (unsigned char *)pchMessage, dwLength - 1, CRC8_INIT_JUDGE);
    pchMessage[dwLength - 1] = ucCRC;
}

uint16_t CRC_INIT_JUDGE = 0xffff;
const  uint16_t  wCRC_Table_JUDGE[256] = {
...
uint16_t Get_CRC16_Check_Sum(uint8_t *pchMessage, uint32_t dwLength, uint16_t wCRC)
{
    ...
    {
        chData = *pchMessage++;
        (wCRC) = ((uint16_t)(wCRC) >> 8) ^ wCRC_Table_JUDGE[((uint16_t)(wCRC) ^
...
void Append_CRC16_Check_Sum(uint8_t * pchMessage, uint32_t dwLength)
{
    ...
    wCRC = Get_CRC16_Check_Sum ( (uint8_t *)pchMessage, dwLength-2, CRC_INIT_JUDGE );
